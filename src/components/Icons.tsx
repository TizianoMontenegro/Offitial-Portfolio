type Weight = { weight?: number };
export const standardWeightLogo = 30;

export function GithubIconDark({ weight = standardWeightLogo }: Weight) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={weight}
            height={weight}
            version="1.1"
            viewBox="0 0 512 512"
            xmlSpace="preserve"
        >
            <path
                fill="#0D2636"
                fillRule="evenodd"
                d="M296.133 354.174c49.885-5.891 102.942-24.029 102.942-110.192 0-24.49-8.624-44.448-22.67-59.869 2.266-5.89 9.515-28.114-2.734-58.947 0 0-18.139-5.898-60.759 22.669-18.139-4.983-38.09-8.163-56.682-8.163-19.053 0-39.011 3.18-56.697 8.163-43.082-28.567-61.22-22.669-61.22-22.669-12.241 30.833-4.983 53.057-2.718 58.947-14.061 15.42-22.677 35.379-22.677 59.869 0 86.163 53.057 104.301 102.942 110.192-6.344 5.452-12.241 15.873-14.507 30.387-12.702 5.438-45.808 15.873-65.758-18.592 0 0-11.795-21.31-34.012-22.669 0 0-22.224-.453-1.813 13.592 0 0 14.96 6.812 24.943 32.653 0 0 13.6 43.089 76.179 29.48v38.543c0 5.906-4.53 12.702-15.865 10.89C96.139 438.977 32.2 354.626 32.2 255.77c0-123.807 100.216-224.022 224.03-224.022 123.347 0 224.023 100.216 223.57 224.022 0 98.856-63.946 182.754-152.828 212.688-11.342 2.266-15.873-4.53-15.873-10.89V395.45c.001-20.873-6.811-34.465-14.966-41.276zM512 256.23C512 114.73 397.263 0 256.23 0 114.73 0 0 114.73 0 256.23 0 397.263 114.73 512 256.23 512 397.263 512 512 397.263 512 256.23z"
                clipRule="evenodd"
            ></path>
        </svg>
    );
}
export function GithubIconLight({ weight = standardWeightLogo }: Weight) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={weight}
            height={weight}
            version="1.1"
            viewBox="0 0 512 512"
            xmlSpace="preserve"
        >
            <path
                fill="#eee"
                fillRule="evenodd"
                d="M296.133 354.174c49.885-5.891 102.942-24.029 102.942-110.192 0-24.49-8.624-44.448-22.67-59.869 2.266-5.89 9.515-28.114-2.734-58.947 0 0-18.139-5.898-60.759 22.669-18.139-4.983-38.09-8.163-56.682-8.163-19.053 0-39.011 3.18-56.697 8.163-43.082-28.567-61.22-22.669-61.22-22.669-12.241 30.833-4.983 53.057-2.718 58.947-14.061 15.42-22.677 35.379-22.677 59.869 0 86.163 53.057 104.301 102.942 110.192-6.344 5.452-12.241 15.873-14.507 30.387-12.702 5.438-45.808 15.873-65.758-18.592 0 0-11.795-21.31-34.012-22.669 0 0-22.224-.453-1.813 13.592 0 0 14.96 6.812 24.943 32.653 0 0 13.6 43.089 76.179 29.48v38.543c0 5.906-4.53 12.702-15.865 10.89C96.139 438.977 32.2 354.626 32.2 255.77c0-123.807 100.216-224.022 224.03-224.022 123.347 0 224.023 100.216 223.57 224.022 0 98.856-63.946 182.754-152.828 212.688-11.342 2.266-15.873-4.53-15.873-10.89V395.45c.001-20.873-6.811-34.465-14.966-41.276zM512 256.23C512 114.73 397.263 0 256.23 0 114.73 0 0 114.73 0 256.23 0 397.263 114.73 512 256.23 512 397.263 512 512 397.263 512 256.23z"
                clipRule="evenodd"
            ></path>
        </svg>
    );
}


export function LinkedinIcon({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="#0078B5"
            fillRule="evenodd"
            strokeLinejoin="round"
            strokeMiterlimit="2"
            clipRule="evenodd"
            viewBox="0 0 512 512"
            width={weight}
            height={weight}
        >
            <path d="M256 0c141.29 0 256 114.71 256 256 0 141.29-114.71 256-256 256C114.71 512 0 397.29 0 256 0 114.71 114.71 0 256 0zm-80.037 399.871V199.95h-66.464v199.921h66.464zm239.62 0V285.225c0-61.409-32.787-89.976-76.509-89.976-35.255 0-51.047 19.389-59.889 33.007V199.95h-66.447c.881 18.757 0 199.921 0 199.921h66.446v-111.65c0-5.976.43-11.95 2.191-16.221 4.795-11.935 15.737-24.299 34.095-24.299 24.034 0 33.663 18.34 33.663 45.204v106.966h66.45zM143.18 103.55c-22.74 0-37.597 14.95-37.597 34.545 0 19.182 14.405 34.544 36.717 34.544h.429c23.175 0 37.6-15.362 37.6-34.544-.43-19.595-14.424-34.545-37.149-34.545z"></path>
        </svg>
    );
}


export function TwitterIcon({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            version="1.1"
            viewBox="0 0 24 24"
            xmlSpace="preserve"
            width={weight}
            height={weight}
        >
            <path
                fill="#1BA8E1"
                d="M1.565 3.052C3.575 5.626 7.172 8.054 12 8.315c-.266-1.117-.292-4.15 1.813-5.41C14.755 2.34 15.646 2 16.696 2c1.32 0 2.402.456 3.554 1.566.938-.138 2.689-.897 3.104-1.195-.27 1.046-1.353 2.39-2.135 2.71.719-.003 2.297-.436 2.781-.691-.671 1.088-1.843 2.13-2.484 2.563.742 6.704-5.182 15.039-13.69 15.047-2.43.002-5.07-.518-7.826-2.105 2.719.32 5.632-.568 7.304-2.106-2.109-.048-4.245-1.482-5.217-3.684 1.057.332 2.031.5 2.609 0C3.71 13.994.522 12.23.522 8.819c.733.611 1.788 1.244 2.608 1.075C1.788 9.093-.163 6.04 1.565 3.052z"
            ></path>
            <path
                fill="#FFF"
                d="M12.043 8.57c-.017-.07-.029-.17-.043-.255-4.806-.273-8.432-2.697-10.435-5.263a4.881 4.881 0 00-.673 2.466c.04-.719.241-1.47.673-2.216 2.01 2.574 5.65 5.007 10.478 5.268zM13.813 3.154c.942-.564 1.833-.904 2.883-.904 1.32 0 2.402.456 3.554 1.566.879-.129 2.458-.8 2.997-1.13.04-.106.08-.213.107-.315-.415.298-2.167 1.057-3.104 1.195C19.098 2.456 18.015 2 16.696 2c-1.05 0-1.941.34-2.883.904-1.616.968-1.973 2.975-1.932 4.363.009-1.37.41-3.202 1.932-4.113z"
                opacity="0.2"
            ></path>
            <path
                fill="#010101"
                d="M21.504 6.711c.009.08.005.162.012.242.643-.437 1.817-1.481 2.484-2.563-.063.033-.146.07-.241.107-.684.953-1.685 1.83-2.255 2.214zM.537 8.84l-.015-.013c0 3.124 2.69 4.931 4.174 5.278.107-.047.21-.104.3-.182-.96-.108-4.299-1.846-4.459-5.083zM7.826 21.75c-2.272.002-4.73-.465-7.292-1.822-.178-.01-.357-.013-.534-.033C2.756 21.482 5.396 22.002 7.826 22c8.037-.008 13.76-7.444 13.745-13.92-.13 6.416-5.811 13.662-13.745 13.67z"
                opacity="0.1"
            ></path>
            <path
                fill="#010101"
                d="M2.243 14.16l-.156-.046c.929 2.105 2.942 3.58 5.217 3.675.091-.074.193-.143.277-.221-2.01-.046-4.31-1.38-5.338-3.408z"
                opacity="0.1"
            ></path>
            <linearGradient
                id="XMLID_96_"
                x1="0.244"
                x2="19.689"
                y1="5.836"
                y2="14.904"
                gradientUnits="userSpaceOnUse"
            >
                <stop offset="0" stopColor="#FFF" stopOpacity="0.2"></stop>
                <stop offset="1" stopColor="#FFF" stopOpacity="0"></stop>
            </linearGradient>
            <path
                fill="url(#XMLID_96_)"
                d="M1.565 3.052C3.575 5.626 7.172 8.054 12 8.315c-.266-1.117-.292-4.15 1.813-5.41C14.755 2.34 15.646 2 16.696 2c1.32 0 2.402.456 3.554 1.566.938-.138 2.689-.897 3.104-1.195-.27 1.046-1.353 2.39-2.135 2.71.719-.003 2.297-.436 2.781-.691-.671 1.088-1.843 2.13-2.484 2.563.742 6.704-5.182 15.039-13.69 15.047-2.43.002-5.07-.518-7.826-2.105 2.719.32 5.632-.568 7.304-2.106-2.109-.048-4.245-1.482-5.217-3.684 1.057.332 2.031.5 2.609 0C3.71 13.994.522 12.23.522 8.819c.733.611 1.788 1.244 2.608 1.075C1.788 9.093-.163 6.04 1.565 3.052z"
            ></path>
        </svg>
    );
}

export function InstagramIcon({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            version="1.1"
            viewBox="0 0 128 128"
            xmlSpace="preserve"
            width={weight}
            height={weight}
        >
            <g>
                <g>
                    <path
                        fill="none"
                        fillRule="evenodd"
                        d="M0 0H128V128H0z"
                        clipRule="evenodd"
                    ></path>
                    <radialGradient
                        id="Instagram_2_"
                        cx="19.111"
                        cy="128.444"
                        r="163.552"
                        gradientUnits="userSpaceOnUse"
                    >
                        <stop offset="0" stopColor="#FFB140"></stop>
                        <stop offset="0.256" stopColor="#FF5445"></stop>
                        <stop offset="0.599" stopColor="#FC2B82"></stop>
                        <stop offset="1" stopColor="#8E40B7"></stop>
                    </radialGradient>
                    <path
                        fill="url(#Instagram_2_)"
                        fillRule="evenodd"
                        d="M105.843 29.837a7.68 7.68 0 11-15.36 0 7.68 7.68 0 0115.36 0zM64 85.333c-11.782 0-21.333-9.551-21.333-21.333 0-11.782 9.551-21.333 21.333-21.333 11.782 0 21.333 9.551 21.333 21.333 0 11.782-9.551 21.333-21.333 21.333zm0-54.198c-18.151 0-32.865 14.714-32.865 32.865 0 18.151 14.714 32.865 32.865 32.865 18.151 0 32.865-14.714 32.865-32.865 0-18.151-14.714-32.865-32.865-32.865zm0-19.603c17.089 0 19.113.065 25.861.373 6.24.285 9.629 1.327 11.884 2.204 2.987 1.161 5.119 2.548 7.359 4.788 2.24 2.239 3.627 4.371 4.788 7.359.876 2.255 1.919 5.644 2.204 11.884.308 6.749.373 8.773.373 25.862s-.065 19.113-.373 25.861c-.285 6.24-1.327 9.629-2.204 11.884-1.161 2.987-2.548 5.119-4.788 7.359-2.239 2.24-4.371 3.627-7.359 4.788-2.255.876-5.644 1.919-11.884 2.204-6.748.308-8.772.373-25.861.373-17.09 0-19.114-.065-25.862-.373-6.24-.285-9.629-1.327-11.884-2.204-2.987-1.161-5.119-2.548-7.359-4.788-2.239-2.239-3.627-4.371-4.788-7.359-.876-2.255-1.919-5.644-2.204-11.884-.308-6.749-.373-8.773-.373-25.861 0-17.089.065-19.113.373-25.862.285-6.24 1.327-9.629 2.204-11.884 1.161-2.987 2.548-5.119 4.788-7.359 2.239-2.24 4.371-3.627 7.359-4.788 2.255-.876 5.644-1.919 11.884-2.204 6.749-.308 8.773-.373 25.862-.373zM64 0C46.619 0 44.439.074 37.613.385 30.801.696 26.148 1.778 22.078 3.36c-4.209 1.635-7.778 3.824-11.336 7.382C7.184 14.3 4.995 17.869 3.36 22.078 1.778 26.149.696 30.801.385 37.613.074 44.439 0 46.619 0 64s.074 19.561.385 26.387c.311 6.812 1.393 11.464 2.975 15.535 1.635 4.209 3.824 7.778 7.382 11.336 3.558 3.558 7.127 5.746 11.336 7.382 4.071 1.582 8.723 2.664 15.535 2.975 6.826.311 9.006.385 26.387.385s19.561-.074 26.387-.385c6.812-.311 11.464-1.393 15.535-2.975 4.209-1.636 7.778-3.824 11.336-7.382 3.558-3.558 5.746-7.127 7.382-11.336 1.582-4.071 2.664-8.723 2.975-15.535.311-6.826.385-9.006.385-26.387s-.074-19.561-.385-26.387c-.311-6.812-1.393-11.464-2.975-15.535-1.636-4.209-3.824-7.778-7.382-11.336-3.558-3.558-7.127-5.746-11.336-7.382C101.851 1.778 97.199.696 90.387.385 83.561.074 81.381 0 64 0z"
                        clipRule="evenodd"
                    ></path>
                </g>
            </g>
        </svg>
    );
}
export function FacebookIcon({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fillRule="evenodd"
            strokeLinejoin="round"
            strokeMiterlimit="2"
            clipRule="evenodd"
            viewBox="0 0 512 512"
            width={weight}
            height={weight}
        >
            <g fillRule="nonzero">
                <path
                    fill="#1877f2"
                    d="M512 256C512 114.615 397.385 0 256 0S0 114.615 0 256c0 127.777 93.616 233.685 216 252.89V330h-65v-74h65v-56.4c0-64.16 38.219-99.6 96.695-99.6 28.009 0 57.305 5 57.305 5v63h-32.281C305.918 168 296 187.733 296 207.978V256h71l-11.35 74H296v178.89C418.385 489.685 512 383.777 512 256z"
                ></path>
                <path
                    fill="#fff"
                    d="M355.65 330L367 256h-71v-48.022c0-20.245 9.917-39.978 41.719-39.978H370v-63s-29.297-5-57.305-5C254.219 100 216 135.44 216 199.6V256h-65v74h65v178.89a257.912 257.912 0 0040 3.11c13.608 0 26.966-1.065 40-3.11V330h59.65z"
                ></path>
            </g>
        </svg>
    );
}


// export function name({ weight = standardWeightLogo }) {}

export function JavascriptIcon({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={weight}
            height={weight}
            enableBackground="new 0 0 512 512"
            viewBox="0 0 512 512"
        >
            <g>
                <path
                    fill="#F0DB4F"
                    d="M26.002 26.001H485.998V485.99899999999997H26.002z"
                ></path>
                <path
                    fill="#323330"
                    d="M276.331 384.759c0 44.767-26.286 65.2-64.586 65.2-34.601 0-54.623-17.865-64.892-39.529l35.218-21.255c6.777 12.013 12.938 22.177 27.826 22.177 14.169 0 23.207-5.544 23.207-27.208V237.21h43.227v147.549zM378.598 449.959c-40.147 0-66.124-19.099-78.754-44.151l35.219-20.332c9.241 15.095 21.356 26.286 42.611 26.286 17.866 0 29.364-8.932 29.364-21.355 0-14.787-11.704-20.021-31.52-28.75l-10.781-4.62c-31.214-13.246-51.853-29.983-51.853-65.2 0-32.447 24.745-57.09 63.248-57.09 27.518 0 47.232 9.549 61.402 34.603l-33.679 21.562c-7.392-13.246-15.401-18.481-27.825-18.481-12.63 0-20.639 8.01-20.639 18.481 0 12.938 8.009 18.176 26.594 26.285l10.78 4.621c36.759 15.71 57.397 31.832 57.397 67.974.002 38.81-30.597 60.167-71.564 60.167z"
                ></path>
            </g>
        </svg>
    )
}
export function TypescriptIcon({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={weight}
            height={weight}
            fill="#007acc"
            enableBackground="new 0 0 24 24"
            version="1.1"
            viewBox="0 0 24 24"
            xmlSpace="preserve"
        >
            <path d="M21.656 0H2.344A2.345 2.345 0 000 2.344v19.312A2.345 2.345 0 002.344 24h19.312A2.345 2.345 0 0024 21.656V2.344A2.345 2.345 0 0021.656 0zm-7.734 13.316h-3.01v8.574H8.527v-8.574H5.531V11.39h8.391v1.926zm7.776 7.054c-.467.835-1.325 1.305-2.238 1.5-1.06.226-2.187.214-3.25.015a4.955 4.955 0 01-1.353-.44v-2.347a4.507 4.507 0 002.898 1.071c.45 0 .951-.056 1.339-.304.266-.17.444-.438.444-.759 0-.437-.316-.767-.657-.997-.499-.336-1.072-.555-1.625-.785-.822-.342-1.434-.76-1.838-1.254-.702-.859-.817-2.231-.275-3.201.463-.828 1.316-1.316 2.216-1.533.981-.236 2.046-.236 3.041-.081a5.72 5.72 0 011.17.304v2.193a3.48 3.48 0 00-.576-.322 4.767 4.767 0 00-1.298-.367c-.445-.06-.914-.07-1.353.033-.346.08-.71.241-.91.55a.793.793 0 00-.125.436c0 .176.046.334.139.473.235.353.649.58 1.02.763.652.32 1.336.56 1.962.931.608.359 1.148.859 1.412 1.525.127.32.191.693.191 1.118-.001.587-.112 1.08-.334 1.478z"></path>
        </svg>
    )
}
export function NextjsIconDark({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={weight}
            height={weight}
            fill="none"
            viewBox="0 0 24 24"
        >
            <g clipPath="url(#clip0_950_641)">
                <path
                    fill="#000"
                    d="M11.214.006c-.052.005-.216.022-.364.033C7.442.347 4.25 2.186 2.226 5.013a11.88 11.88 0 00-2.118 5.243c-.096.66-.108.854-.108 1.748s.012 1.089.108 1.748c.652 4.507 3.86 8.293 8.209 9.696.779.251 1.6.422 2.533.526.364.04 1.936.04 2.3 0 1.611-.179 2.977-.578 4.323-1.265.207-.105.247-.134.219-.157-.02-.014-.899-1.194-1.955-2.62l-1.919-2.593-2.404-3.559a342.499 342.499 0 00-2.422-3.556c-.009-.003-.018 1.578-.023 3.51-.007 3.38-.01 3.516-.052 3.596a.426.426 0 01-.206.213c-.075.038-.14.045-.495.045H7.81l-.108-.068a.44.44 0 01-.157-.172l-.05-.105.005-4.704.007-4.706.073-.092a.644.644 0 01.174-.143c.096-.047.133-.051.54-.051.478 0 .558.018.682.154.035.038 1.337 2 2.895 4.362l4.734 7.172 1.9 2.878.097-.063a12.318 12.318 0 002.465-2.163 11.947 11.947 0 002.825-6.135c.096-.66.108-.854.108-1.748s-.012-1.088-.108-1.748C23.24 5.75 20.032 1.963 15.683.56a12.6 12.6 0 00-2.498-.523c-.226-.024-1.776-.05-1.97-.03zm4.913 7.26a.473.473 0 01.237.276c.018.06.023 1.365.018 4.305l-.007 4.218-.743-1.14-.746-1.14v-3.066c0-1.983.009-3.097.023-3.151a.478.478 0 01.232-.296c.097-.05.132-.054.5-.054.347 0 .408.005.486.047z"
                ></path>
            </g>
            <defs>
                <clipPath id="clip0_950_641">
                    <path fill="#fff" d="M0 0H24V24H0z"></path>
                </clipPath>
            </defs>
        </svg>
    )
}
export function NextjsIconLight({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={weight}
            height={weight}
            fill="none"
            viewBox="0 0 24 24"
        >
            <g clipPath="url(#clip0_950_641)">
                <path
                    fill="#eee"
                    d="M11.214.006c-.052.005-.216.022-.364.033C7.442.347 4.25 2.186 2.226 5.013a11.88 11.88 0 00-2.118 5.243c-.096.66-.108.854-.108 1.748s.012 1.089.108 1.748c.652 4.507 3.86 8.293 8.209 9.696.779.251 1.6.422 2.533.526.364.04 1.936.04 2.3 0 1.611-.179 2.977-.578 4.323-1.265.207-.105.247-.134.219-.157-.02-.014-.899-1.194-1.955-2.62l-1.919-2.593-2.404-3.559a342.499 342.499 0 00-2.422-3.556c-.009-.003-.018 1.578-.023 3.51-.007 3.38-.01 3.516-.052 3.596a.426.426 0 01-.206.213c-.075.038-.14.045-.495.045H7.81l-.108-.068a.44.44 0 01-.157-.172l-.05-.105.005-4.704.007-4.706.073-.092a.644.644 0 01.174-.143c.096-.047.133-.051.54-.051.478 0 .558.018.682.154.035.038 1.337 2 2.895 4.362l4.734 7.172 1.9 2.878.097-.063a12.318 12.318 0 002.465-2.163 11.947 11.947 0 002.825-6.135c.096-.66.108-.854.108-1.748s-.012-1.088-.108-1.748C23.24 5.75 20.032 1.963 15.683.56a12.6 12.6 0 00-2.498-.523c-.226-.024-1.776-.05-1.97-.03zm4.913 7.26a.473.473 0 01.237.276c.018.06.023 1.365.018 4.305l-.007 4.218-.743-1.14-.746-1.14v-3.066c0-1.983.009-3.097.023-3.151a.478.478 0 01.232-.296c.097-.05.132-.054.5-.054.347 0 .408.005.486.047z"
                ></path>
            </g>
            <defs>
                <clipPath id="clip0_950_641">
                    <path fill="#fff" d="M0 0H24V24H0z"></path>
                </clipPath>
            </defs>
        </svg>
    )
}
export function ReactIcon({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            fillRule="evenodd"
            strokeLinejoin="round"
            strokeMiterlimit="2"
            clipRule="evenodd"
            viewBox="0 0 128 128"
            width={weight}
            height={weight}
        >
            <g fill="#61DAFB" transform="translate(-16.308 4.572) scale(1.25305)">
                <circle cx="64" cy="47.5" r="9.3"></circle>
                <path
                    fillRule="nonzero"
                    d="M64 81.7C71.3 88.8 78.5 93 84.3 93c1.9 0 3.7-.4 5.2-1.3 5.2-3 7.1-10.5 5.3-21.2-.3-1.9-.7-3.8-1.2-5.8 2-.6 3.8-1.2 5.6-1.8 10.1-3.9 15.7-9.3 15.7-15.2 0-6-5.6-11.4-15.7-15.2-1.8-.7-3.6-1.3-5.6-1.8.5-2 .9-3.9 1.2-5.8 1.7-10.9-.2-18.5-5.4-21.5-1.5-.9-3.3-1.3-5.2-1.3-5.7 0-13 4.2-20.3 11.3C56.7 6.3 49.5 2.1 43.7 2.1c-1.9 0-3.7.4-5.2 1.3-5.2 3-7.1 10.5-5.3 21.2.3 1.9.7 3.8 1.2 5.8-2 .6-3.8 1.2-5.6 1.8-10.1 3.9-15.7 9.3-15.7 15.2 0 6 5.6 11.4 15.7 15.2 1.8.7 3.6 1.3 5.6 1.8-.5 2-.9 3.9-1.2 5.8-1.7 10.7.2 18.3 5.3 21.2 1.5.9 3.3 1.3 5.2 1.3 5.8.2 13-4 20.3-11zm-5.6-13.5c1.8.1 3.7.1 5.6.1 1.9 0 3.8 0 5.6-.1-1.8 2.4-3.7 4.6-5.6 6.7-1.9-2.1-3.8-4.3-5.6-6.7zM46 57.9c1 1.7 1.9 3.3 3 4.9-3.1-.4-6-.9-8.8-1.5.9-2.7 1.9-5.5 3.1-8.3.8 1.6 1.7 3.3 2.7 4.9zm-5.8-24.1c2.8-.6 5.7-1.1 8.8-1.5-1 1.6-2 3.2-3 4.9-1 1.7-1.9 3.3-2.7 5-1.3-2.9-2.3-5.7-3.1-8.4zm5.5 13.7c1.3-2.7 2.7-5.4 4.3-8.1 1.5-2.6 3.2-5.2 4.9-7.8 3-.2 6-.3 9.1-.3 3.2 0 6.2.1 9.1.3 1.8 2.6 3.4 5.2 4.9 7.8 1.6 2.7 3 5.4 4.3 8.1-1.3 2.7-2.7 5.4-4.3 8.1-1.5 2.6-3.2 5.2-4.9 7.8-3 .2-6 .3-9.1.3-3.2 0-6.2-.1-9.1-.3-1.8-2.6-3.4-5.2-4.9-7.8-1.6-2.7-3-5.4-4.3-8.1zm39.1-5.4l-2.7-5c-1-1.7-1.9-3.3-3-4.9 3.1.4 6 .9 8.8 1.5-.9 2.8-1.9 5.6-3.1 8.4zm0 10.8c1.2 2.8 2.2 5.6 3.1 8.3-2.8.6-5.7 1.1-8.8 1.5 1-1.6 2-3.2 3-4.9.9-1.5 1.8-3.2 2.7-4.9zm2.3 34.7c-.8.5-1.8.7-2.9.7-4.9 0-11-4-17-10 2.9-3.1 5.7-6.6 8.5-10.5 4.7-.4 9.2-1.1 13.4-2.1.5 1.8.8 3.6 1.1 5.4 1.4 8.5.3 14.6-3.1 16.5zm5.2-52.7c11.2 3.2 17.9 8.1 17.9 12.6 0 3.9-4.6 7.8-12.7 10.9-1.6.6-3.4 1.2-5.2 1.7-1.3-4.1-2.9-8.3-4.9-12.6 2-4.3 3.7-8.5 4.9-12.6zm-8-28.2c1.1 0 2 .2 2.9.7 3.3 1.9 4.5 7.9 3.1 16.5-.3 1.7-.7 3.5-1.1 5.4-4.2-.9-8.7-1.6-13.4-2.1-2.7-3.9-5.6-7.4-8.5-10.5 6-5.9 12.1-10 17-10zM69.6 26.8c-1.8-.1-3.7-.1-5.6-.1-1.9 0-3.8 0-5.6.1 1.8-2.4 3.7-4.6 5.6-6.7 1.9 2.1 3.8 4.4 5.6 6.7zM40.9 7.4c.8-.5 1.8-.7 2.9-.7 4.9 0 11 4 17 10-2.9 3.1-5.7 6.6-8.5 10.5-4.7.4-9.2 1.1-13.4 2.1-.5-1.8-.8-3.6-1.1-5.4-1.4-8.5-.3-14.5 3.1-16.5zm-5.2 52.7C24.5 56.9 17.8 52 17.8 47.5c0-3.9 4.6-7.8 12.7-10.9 1.6-.6 3.4-1.2 5.2-1.7 1.3 4.1 2.9 8.3 4.9 12.6-2 4.3-3.7 8.6-4.9 12.6zm2.1 11c.3-1.7.7-3.5 1.1-5.4 4.2.9 8.7 1.6 13.4 2.1 2.7 3.9 5.6 7.4 8.5 10.5-6 5.9-12.1 10-17 10-1.1 0-2-.2-2.9-.7-3.4-1.9-4.5-8-3.1-16.5z"
                ></path>
            </g>
        </svg>
    );
}
export function ReduxIcon({ weight = standardWeightLogo }) {
    return (
        <svg xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            width={weight}
            height={weight}
            fill="#764abc">
            <path d="M16.633 16.504c.869-.075 1.543-.84 1.499-1.754-.046-.914-.795-1.648-1.708-1.648h-.061a1.71 1.71 0 00-1.648 1.769c.03.479.226.869.494 1.153-1.048 2.038-2.621 3.536-5.004 4.795-1.603.838-3.296 1.154-4.944.929-1.378-.194-2.456-.81-3.116-1.798-.988-1.499-1.078-3.116-.255-4.734.601-1.169 1.499-2.023 2.099-2.443a9.96 9.96 0 01-.42-1.542C-.867 14.408-.416 18.752.932 20.805c1.004 1.498 3.057 2.456 5.304 2.456.599 0 1.229-.044 1.843-.194 3.896-.749 6.847-3.086 8.54-6.532l.014-.031zm5.348-3.746c-2.321-2.727-5.738-4.225-9.634-4.225h-.51c-.253-.554-.837-.899-1.497-.899h-.045c-.943 0-1.678.81-1.647 1.753.03.898.794 1.648 1.708 1.648h.074a1.69 1.69 0 001.498-1.049h.555c2.309 0 4.495.674 6.488 1.992 1.527 1.004 2.622 2.322 3.236 3.896.538 1.288.509 2.547-.045 3.597-.854 1.647-2.293 2.517-4.195 2.517-1.199 0-2.367-.375-2.967-.644-.359.298-.959.793-1.394 1.093 1.318.598 2.652.943 3.94.943 2.922 0 5.093-1.647 5.918-3.236.898-1.798.824-4.824-1.469-7.416l-.014.03zM6.49 17.042a1.724 1.724 0 001.708 1.648h.06a1.688 1.688 0 001.648-1.768c0-.899-.779-1.647-1.693-1.647h-.061c-.06 0-.149 0-.225.029-1.243-2.098-1.768-4.346-1.572-6.771.119-1.828.719-3.417 1.797-4.735.899-1.124 2.592-1.679 3.746-1.708 3.236-.061 4.585 3.971 4.689 5.574l1.498.449c-.345-4.914-3.4-7.492-6.322-7.492-2.742 0-5.273 1.993-6.293 4.915-1.393 3.896-.479 7.641 1.229 10.638-.149.195-.239.539-.209.868z"></path>
        </svg>
    );
}
export function HtmlIcon({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={weight}
            height={weight}
            version="1"
            viewBox="0 0 32 32"
            xmlSpace="preserve"
        >
            <path
                fill="#E44D26"
                d="M27.377 28.889L16.001 32 4.625 28.889 2 0 30.002 0z"
            ></path>
            <path fill="#FF6C39" d="M16 2L16 29.75 25.232 27.008 27.688 2z"></path>
            <path
                fill="#FFF"
                d="M24.363 6L7.607 6 8 10 8.619 17 19.503 17 19.148 20.984 16 21.99 12.857 20.984 12.648 19 8.803 19 9.262 23.987 16 25.99 22.728 23.986 23.718 13 12.252 13 12.002 10 24 10z"
            ></path>
        </svg>
    );
}
export function CssIcon({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={weight}
            height={weight}
            version="1"
            viewBox="0 0 32 32"
            xmlSpace="preserve"
        >
            <path
                fill="#1F62AE"
                d="M27.377 28.889L16.001 32 4.625 28.889 2 0 30.002 0z"
            ></path>
            <path fill="#347DC6" d="M16 2L16 29.75 25.232 27.008 27.688 2z"></path>
            <path
                fill="#FFF"
                d="M24.363 6L7.607 6 8 10 16 10 8.25 12.99 8.619 17 19.502 17 19.158 21 16 21.99 12.861 20.984 12.533 19 8.803 19 9.262 23.987 16 25.99 22.728 23.986 23.719 12.99 16.026 12.99 24 10z"
            ></path>
        </svg>
    );
}
export function PythonIcon({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={weight}
            height={weight}
            enableBackground="new 0 0 512 512"
            viewBox="0 0 512 512"
        >
            <g>
                <path
                    fill="#347AB4"
                    d="M194.005 240.252h105.054c29.216 0 52.529-24.101 52.529-53.414V86.603c0-28.525-24.002-49.871-52.529-54.691-35.214-5.804-73.479-5.509-105.054.097-44.462 7.87-52.527 24.298-52.527 54.693v21.453H246.63v31.959H102.032c-30.591 0-57.349 18.394-65.709 53.315-9.64 40.035-10.032 65.02 0 106.826 7.477 31.085 25.28 53.314 55.872 53.314h36.101v-48.001c0-34.723 30.002-65.316 65.709-65.316z"
                ></path>
                <path
                    fill="#275C87"
                    d="M128.296 363.569H92.195c-32.842 0-56.137-21.655-65.595-60.976-10.146-42.276-10.145-69.37 0-111.505 4.521-18.885 14.243-34.559 28.112-45.314 13.21-10.245 29.573-15.66 47.319-15.66H236.63v-11.959h-95.152c-5.523 0-10-4.477-10-10V86.702c0-13.954 1.423-29.889 11.979-42.278 9.452-11.094 24.504-17.96 48.806-22.261 16.512-2.932 35.127-4.482 53.829-4.482 18.655 0 37.023 1.469 54.594 4.365 16.345 2.762 31.383 10.299 42.385 21.229 11.941 11.865 18.519 27.253 18.519 43.329v100.235c0 34.967-28.051 63.414-62.529 63.414H194.005c-29.677 0-55.709 25.849-55.709 55.316v48.001c0 5.522-4.477 9.999-10 9.999zm-26.264-213.455c-28.213 0-49.142 17.063-55.984 45.643-9.526 39.563-9.525 62.475-.001 102.165 5.008 20.821 17.081 45.647 46.148 45.647h26.101v-38.001c0-19.263 8.159-38.487 22.386-52.743 14.527-14.557 33.464-22.573 53.323-22.573h105.054c23.451 0 42.529-19.476 42.529-43.414V86.603c0-24.529-22.233-41.12-44.195-44.831-16.46-2.713-33.733-4.092-51.302-4.092-17.549 0-34.956 1.444-50.337 4.175-39.908 7.064-44.275 19.689-44.275 44.847v11.453h95.152c5.523 0 10 4.477 10 10v31.959c0 5.523-4.477 10-10 10H102.032z"
                ></path>
                <path
                    fill="#FFCA1D"
                    d="M474.975 209.372c-7.575-30.395-21.935-53.315-52.527-53.315h-39.443v46.626c0 36.199-30.692 66.691-65.71 66.691H212.24c-28.724 0-52.528 24.594-52.528 53.414v100.138c0 28.524 24.788 45.246 52.528 53.413 33.247 9.737 65.215 11.509 105.054 0 26.462-7.675 52.529-23.116 52.529-53.413v-20.997H264.867v-32.417h157.58c30.593 0 41.904-21.346 52.527-53.314 11.018-32.954 10.527-64.628.001-106.826z"
                ></path>
                <path
                    fill="#BF9816"
                    d="M264.431 494.32c-17.747 0-35.738-2.743-55.001-8.385-38.515-11.339-59.718-33.715-59.718-63.01V322.788c0-34.374 28.634-63.414 62.528-63.414h105.054c29.678 0 55.71-26.491 55.71-56.691v-46.626c0-5.523 4.478-10 10-10h39.443c17.276 0 31.546 6.279 42.412 18.664 8.734 9.955 15.217 23.77 19.818 42.232 11.191 44.863 11.126 78.483-.219 112.415-10.11 30.424-23.23 60.144-62.012 60.144h-147.58v12.417h94.956c5.522 0 10 4.478 10 10v20.997c0 20.844-10.365 48.696-59.744 63.018-19.515 5.637-37.714 8.376-55.647 8.376zM212.24 279.374c-23.053 0-42.528 19.881-42.528 43.414v100.138c0 26.51 28.415 38.834 45.353 43.82 17.639 5.166 33.33 7.574 49.366 7.574 16.028 0 32.413-2.482 50.088-7.589 30.059-8.718 45.305-23.457 45.305-43.806v-10.997h-94.956c-5.522 0-10-4.478-10-10v-32.417c0-5.522 4.478-10 10-10h157.58c21.148 0 31.204-10.857 43.037-46.468 10.189-30.476 10.126-59.805-.213-101.252-7.883-31.631-21.09-45.735-42.824-45.735h-29.443v36.626c0 41.571-34.671 76.691-75.71 76.691H212.24z"
                ></path>
                <path
                    fill="#FFF"
                    d="M187.414 89.594c-10.479 0-18.975-8.591-18.975-19.164.094-10.668 8.496-19.259 18.975-19.259 10.386 0 18.976 8.685 18.976 19.259 0 10.573-8.496 19.164-18.976 19.164zM314.908 460.076c-10.479 0-18.976-8.591-18.976-19.164.095-10.668 8.496-19.259 18.976-19.259 10.385 0 18.976 8.685 18.976 19.259 0 10.573-8.496 19.164-18.976 19.164z"
                ></path>
            </g>
        </svg>
    );
}
export function SqlIcon({ weight = standardWeightLogo }) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={weight}
            height={weight}
            viewBox="0 0 32 32"
        >
            <g fill="none" fillRule="evenodd" stroke="none" strokeWidth="1">
                <g fill="#157EFB">
                    <path d="M17.676 19.883a2.017 2.017 0 01-.68.117h-.991A2.003 2.003 0 0114 17.994v-2.988c0-1.12.898-2.006 2.005-2.006h.99c1.111 0 2.005.898 2.005 2.006v2.988c0 .505-.183.963-.485 1.313l.753.754-.707.707-.885-.885zM16.793 19h-.794A.996.996 0 0115 18v-3c0-.556.447-1 .999-1h1.002c.556 0 .999.448.999 1v3c0 .225-.073.43-.196.597l-1.365-1.365-.707.707L16.792 19zm-8.786-9A3.003 3.003 0 005 12.999V20A2.996 2.996 0 008.007 23h16.986A3.003 3.003 0 0028 20.001V13A2.996 2.996 0 0024.993 10H8.007zm1.998 3A1.998 1.998 0 008 15c0 1.105.888 2 2 2h.99c.558 0 1.01.444 1.01 1 0 .552-.443 1-.999 1H9.999C9.447 19 9 18.544 9 18.004v-.01H8v.005A2 2 0 0010.005 20h.99A1.998 1.998 0 0013 18c0-1.105-.888-2-2-2h-.99C9.451 16 9 15.556 9 15c0-.552.443-1 .999-1h1.002c.552 0 .999.453.999 1h1a2 2 0 00-2.005-2h-.99zM25 19v1h-5v-7h1v6h4z"></path>
                </g>
            </g>
        </svg>
    );
}
